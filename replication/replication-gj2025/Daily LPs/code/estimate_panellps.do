...
restore
foreach depvar_curr of global lp_depvar_timeseries	{
...
drop time
reshape long ${lp_depvar_panel}, i(time_td) j(country, string)
rename time_td time
encode country, gen(country_num)
xtset country_num time

/* La función btime no está en el paquete de replicacion, la voy a reemplazar
gen time_td = time
format time_td %td
btime, replacetvar
xtset country_num time
...
gen time_td = time
format time_td %td
egen bday = group(time_td)   
xtset country_num bday


foreach depvar_curr of global lp_depvar_panel	{
	
	forvalues h=0(1)$lp_hor	{	
			
		preserve
		foreach shockvar_curr of global lp_shock	{
			quietly xtscc f`h'.`depvar_curr' l(1/${lp_ownlag}).`depvar_curr' l(1/${lp_controlslag}).(${lp_controls}) ${contemp_controls} `shockvar_curr' if time_td>td(${mindate}) & time_td<td(${maxdate}), fe
			gen irfj_`depvar_curr'_`shockvar_curr' = _b[`shockvar_curr']
			gen irfj_`depvar_curr'_`shockvar_curr'_u90 = _b[`shockvar_curr'] + 1.64*_se[`shockvar_curr']
			gen irfj_`depvar_curr'_`shockvar_curr'_l90 = _b[`shockvar_curr'] - 1.64*_se[`shockvar_curr']
			gen irfj_`depvar_curr'_`shockvar_curr'_u68 = _b[`shockvar_curr'] + _se[`shockvar_curr']
